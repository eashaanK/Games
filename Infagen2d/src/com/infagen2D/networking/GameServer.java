...
public class GameServer extends Thread {
	...
	private void readData(byte[] buffer) throws IOException {
		...
		case INVALID:
			break;
		case LOGIN:
			handleLogin(messageParts[1], Integer.parseInt(messageParts[2].trim()), Integer.parseInt(messageParts[3].trim()), request);
			break;
		case DISCONNECT:
			handleDisconnect(messageParts[1], request.getAddress(), request.getPort());
			break;
		case MOVE:
			handleMove(messageParts[1], Integer.parseInt(messageParts[2].trim()), Integer.parseInt(messageParts[3].trim()), request);
			break;
		case MESSAGE:
	...
	private void handleMove(String usernameWhoMoved, int x, int y, DatagramPacket request) {
	 ...
	 * @param messageParts
	 */
	private void handleLogin(String username, int sX, int sY, DatagramPacket request) {
		//System.out.println(username + " joined");
		debugger.println(username + " joined");

		try {
			this.connectedPlayers.add(new PlayerMP(null, username, sX, sY, request.getAddress(), request.getPort()));
	...
	private void sendToAll(PacketType packetType, String message,
		...
		for (PlayerMP p : this.connectedPlayers) {
			if( ! p.getName().equals(usernameToAvoid)){
